...
var sha3 = exports.sha3 = function (data) {
  ...
  d.update(data);
  return new Buffer(d.digest('binary'), 'binary');
}
...
var namedHash = exports.namedHash = function (hash, data) {
  var hashed = data;
  for (var i in hash) {
    var h = hash[i];
    if (h == 'sha3') hashed = sha3(hashed);
    else if (h == 'sha256') hashed = sha256(hashed); 
    else if (h == 'ripemd160') hashed = ripe160(hashed);
    else throw new Error('Named hash type unknown: '.concat(h));
  }
  return hashed;
}

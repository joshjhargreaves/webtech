...
var namedHash       = imports.namedHash || util.namedHash;
...
Connection.prototype.handleMessage = function(message) {
  ...
  try {
    ...
    case 'version':
      ...
      this.sendVer = Math.min(message.version, network.protocolVersion);
      if (message.version < 209) {
        this.recvVer = Math.min(message.version, network.protocolVersion);
      } else {
...
Connection.prototype.sendVersion = function () {
  //var protocolVersion = network.protocolVersion
  var protocolVersion = network.protocolVersion;
  //var subversion = '/BitcoinX:0.1/';
  var subversion = network.subversion;

  var put = new Put();
  put.word32le(protocolVersion); // version
...
Connection.prototype.sendMessage = function (command, payload) {
  try {
    ...
    var checksum;
    if (this.sendVer >= 209) {
      checksum = namedHash(network.hashes.message, payload).slice(0, 4);
    } else {
      checksum = new Buffer([]);
    }
...
Connection.prototype.processData = function () {
  ...
  if (checksum !== null) {
    var checksumConfirm = namedHash(network.hashes.message, payload).slice(0, 4);
